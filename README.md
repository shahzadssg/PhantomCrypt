...
| `admissibility_check.ipynb` | Tests the cover-admissibility predicate on real text. **Covers conditions 1--3 only** -- see below. |
...
The lemma labels were renamed to keep these apart. 
...
`is_admissible` in the notebook, and `admissibility_check.ipynb`, implement **1--3**. Condition 4 was added later and is **not** in those files; use `condition4_frequency.py` (drop-in `is_admissible_v2`).
